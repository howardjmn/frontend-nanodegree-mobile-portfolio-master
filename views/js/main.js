...
var resizePizzas = function(size) { 
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        ...
        return;
      case "2":
        document.getElementById("pizzaSize").innerHTML = "Medium"; // changed querySelector to getElementById
        return;
      case "3":
        document.getElementById("pizzaSize").innerHTML = "Large"; // changed querySelector to getElementById
        return;
      default:
  ...
  function changePizzaSizes(size) {
    // move logic inside sizeSwitcher function to changePizzaSizes because both receive the same parameter "size" 
    // and also to save time calling sizeSwitcher function, also inside each case there's a variable newWidth that will receive
    // the pencentage (absolute) depending on the case, doing that we avoid FSL.
    switch(size) {
        case "1":
          var newWidth = 25; // changed from .25 to 25 (absolute percentages) as below we change from pixes to percentages
          break;
        case "2":
          var newWidth = 33.33; // changed from .33 to 33.33 (absolute percentages) as below we change from pixes to percentages
          break;
        case "3":
          var newWidth = 50; // changed from .5 to 50 (absolute percentages) as below we change from pixes to percentages
          break;
        default:
...
var pizzasDiv = document.getElementById("randomPizzas"); // move outside the "for" loop to prevent creating it each time.
for (var i = 2; i < 100; i++) {
